...
def create_app(test_config=None):
  ...
  def get_questions():
    ...
    if len(questions) == 0:
      ...
      data = dict()
      # add questions key to the dictionary with value of formatted_questions
      ...
      # create key of id with value of type in the dictionary on the categories key
      for category in formatted_categories:
        data['categories'][category['id']] = category['type']
      # https://knowledge.udacity.com/questions/809673 set current_category to
      # empty value per Udacity mentor as there is more than one category returned for all questions
  ...
  def search_questions():
    try:
      ...
      questions = Question.query.filter(
          Question.question.ilike('%' + search_term + '%')).order_by(Question.id).all()
  ...
  def get_questions_by_category(category_id):
    try:
      ...
      # set current_category to empty val if category is None, otherwise set to category type
      current_category = None if category is None else category.format()[
          'type']
      # cast int to str as category id is an int on the category model and a str on the question model
      questions = Question.query.filter(
  ...
  def get_questions_for_quiz():
    try:
      ...
      # pick a random number in the range of the len of the non_previous_questions arr - 1
